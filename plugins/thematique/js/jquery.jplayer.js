...
(function ($, undefined) {
	...
	$.jPlayer.pause = function () {
		$.each(
			$.jPlayer.prototype.instances, function (i, element) {
	...
	$.jPlayer.prototype = {
		...
		options: { // Instanced in $.jPlayer() constructor
			...
			repeat: function (event) {
				...
				if (event.jPlayer.options.loop) {
					$(this).unbind(".jPlayerRepeat").bind(
						$.jPlayer.event.ended + ".jPlayer.jPlayerRepeat", function () {
							$(this).jPlayer("play");
						}
					);
				} else {
		...
		_init: function () {
			...
			$.each(
				this.options.supplied.toLowerCase().split(","), function (index1, value1) {
					var format = value1.replace(/^\s+|\s+$/g, ""); //trim
					if (self.format[format]) { // Check format is valid.
						var dupFound = false;
						$.each(
							self.formats, function (index2, value2) {
			...
			$.each(
				this.options.solution.toLowerCase().split(","), function (index1, value1) {
					var solution = value1.replace(/^\s+|\s+$/g, ""); //trim
					if (self.solution[solution]) { // Check solution is valid.
						var dupFound = false;
						$.each(
							self.solutions, function (index2, value2) {
								// Check for duplicates
								if (solution === value2) {
									dupFound = true;
									return false;
			...
			$.each(
				$.jPlayer.event, function (eventName, eventType) {
			...
			$.each(
				this.solutions, function (solutionPriority, solution) {
					if (solutionPriority === 0) {
						...
						$.each(
							self.formats, function (formatPriority, format) {
		...
		_addHtmlEventListeners: function (mediaElement, entity) {
			...
			$.each(
				$.jPlayer.htmlEvent, function (i, eventType) {
					mediaElement.addEventListener(
						this, function () {
							if (entity.gate) {
		...
		setMedia: function (media) {
			...
			*/
			...
			$.each(
				this.formats, function (formatPriority, format) {
					var isVideo = self.format[format].media === 'video';
					$.each(
						self.solutions, function (solutionPriority, solution) {
		...
		pauseOthers: function () {
			var self = this;
			$.each(
				this.instances, function (i, element) {
		...
		_cssSelectorAncestor: function (ancestor) {
			...
			this._addUiClass();
			$.each(
				this.options.cssSelector, function (fn, cssSel) {
			...
			var self = this,
				...
				handler = function () {
					self.css.jq.gui.fadeIn(
						self.options.autohide.fadeIn, function () {
							clearTimeout(self.internal.autohideId);
							self.internal.autohideId = setTimeout(
								function () {
		...
		_emulateHtmlBridge: function () {
			...
			$.each(
				$.jPlayer.emulateMethods.split(/\s+/g), function (i, name) {
			...
			$.each(
				$.jPlayer.event, function (eventName, eventType) {
					...
					$.each(
						$.jPlayer.reservedEvent.split(/\s+/g), function (i, name) {
							if (name === eventName) {
								nativeEvent = false;
								return false;
							}
					...
					if (nativeEvent) {
						self.element.bind(
							eventType + ".jPlayer.jPlayerHtml", function () {
		...
		_emulateHtmlUpdate: function () {
			...
			$.each(
				$.jPlayer.emulateStatus.split(/\s+/g), function (i, name) {
					self.internal.domNode[name] = self.status[name];
				}
			);
			$.each(
				$.jPlayer.emulateOptions.split(/\s+/g), function (i, name) {
		...
		_destroyHtmlBridge: function () {
			...
			$.each(
				emulated.split(/\s+/g), function (i, name) {
