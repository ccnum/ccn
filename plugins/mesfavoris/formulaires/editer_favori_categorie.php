...
function formulaires_editer_favori_categorie_charger_dist($objet = '', $id_objet = 0, $categorie = '') {
	...
	$row = sql_fetsel(
		'categorie',
		'spip_favoris',
		['id_auteur = ' . $id_auteur, 'id_objet = ' . intval($id_objet), 'objet = ' . sql_quote($objet)]
	);
	$contexte = [
...
function formulaires_editer_favori_categorie_traiter_dist($objet = '', $id_objet = 0, $categorie = '') {
	...
	if ($id_auteur = intval($GLOBALS['visiteur_session']['id_auteur'])) {
		...
		if (_request('categoriser') !== null) {
			$res['id_favori'] = mesfavoris_categoriser($id_objet, $objet, $id_auteur, _request('categorie'));
			if ($res['id_favori']) {
