...
function critere_mesfavoris_dist($idb, &$boucles, $crit) {
	...
	$objet = objet_type($primary, $boucle->sql_serveur);
	...
	$type = !isset($crit->param[0][0]) ? "'$not'"
		: calculer_liste([$crit->param[0][0]], [], $boucles, $boucle->id_parent);
 ...
 * @return array Les id des objets à éviter
 */
function prepare_mesfavoris($objet, $type, $server = '') {
	$objets_favoris = sql_allfetsel(
		'id_objet',
		'spip_favoris',
		'objet=' . sql_quote($objet) . ' AND id_auteur=' . intval($GLOBALS['visiteur_session']['id_auteur'])
	);
	$objet = [];
