...
function creer_histoire($id) {
  ...
  //Rubrique
	...
	sql_update('spip_rubriques', ['`statut`' => "'publie'"], 'id_rubrique=' . intval($id_rub));
...
function valider_chapitre($id_article, $id_rubrique) {
	  ...
	  //mail
		...
		//$message = "Bonjour,\r\n\r\nMerci d'avoir participé au petit fablab d'écriture !\r\nAccédez dès maintenant à votre chapitre en ligne : http://fictions.laclasse.com/spip.php?scenario=jeu&page=lecture&id_rubrique=".$id_rubrique.". Un deuxième message vous préviendra lorsque votre histoire sera disponible.\r\n\r\nA très bientôt";
		$message = "Bonjour,\n\nMerci d’avoir participé au petit fablab d’écriture !\n\nAccédez dès maintenant à votre chapitre en ligne : http://fictions.laclasse.com/spip.php?scenario=jeu&page=lecture&id_rubrique=" . $id_rubrique . "\n\nUn deuxième message vous préviendra lorsque votre histoire sera disponible.\n\n A très bientôt,\nL’équipe du petit fablab d’écriture\n\nSuivez nos actualités sur Twitter @petitfablab ou sur le blog https://petit-fablab-ecriture.tumblr.com/ \n\nLe petit fablab d'écriture est un dispositif imaginé par Erasme, laboratoire d'innovation ouverte de la Métropole de Lyon, en collaboration avec la Villa Gillet. \n\n Une question ? Une remarque ? Contactez-nous à l'adresse  petitfablab@gmail.com";
	  ...
	  //Si 5ème chapitre
		...
		//$message = "Bonjour à tous,\r\n\r\nFélicitations votre histoire est en ligne.\r\nDiscutez de l'édition de votre histoire avec vos co-auteurs par retour de mail : http://fictions.laclasse.com/spip.php?scenario=jeu&page=lecture&id_rubrique=".$id_rubrique."\r\n\r\nA très bientôt";
...
function balise_ANNEE_ACTUELLE_dist($p) {
	if (date('m') >= 9) { $p->code = date('Y');
	} else { $p->code = date('Y') - 1;
	}
	return $p;
}
...
function afficher_options_date($annee, $mois, $annee_scolaire) {
  if (date('m') >= 9) { $annee_actuelle = date('Y');
  } else { $annee_actuelle = date('Y') - 1;
  }
