...
function notation_insert_head_css($flux) {
	$flux .= '<link rel="stylesheet" href="' . timestamp(find_in_path('css/notation.v3.css')) . '" type="text/css" media="all" />';
	return $flux;
}
 ...
 * @return mixed
 */
function notation_nospam_lister_formulaires($formulaires) {

	$formulaires[] = 'jaime';
	$formulaires[] = 'jaime_coeur';
	$formulaires[] = 'jaime_jaimepas';
...
function notation_affichage_final($flux) {
	...
	$incHead = '';
	$incHead .= "<script src='" . timestamp(find_in_path('javascript/jquery.MetaData.js')) . "' type='text/javascript'></script>\n";
	$incHead .= "<script src='" . timestamp(find_in_path('javascript/jquery.rating.js')) . "' type='text/javascript'></script>\n";
	$incHead .= "<script src='" . timestamp(find_in_path('javascript/notation.js')) . "' type='text/javascript'></script>\n";
	if (function_exists('compacte_head')) {
		$incHead = trim(compacte_head($incHead));
	}
...
function notation_afficher_config_objet($flux) {
	if (
		...
		if (autoriser('moderernote', $type, $id)) {
			...
			$flux['data'] .= recuperer_fond('prive/configurer/configurer_note', ['id_objet' => $id, 'objet' => objet_type(table_objet($type))]);
		}
	}
	return $flux;
}
...
function notation_pre_insertion($flux) {
	if ($flux['args']['table'] == 'spip_articles') {
		...
		$flux['args']['data']['accepter_note'] = substr(lire_config('notations_publics'), 0, 3);
	}
	return $flux;
}
