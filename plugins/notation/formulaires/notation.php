...
function formulaires_notation_charger_dist($objet, $id_objet) {
	...
	if (!autoriser('modifier', 'notation', $id_notation, null, ['objet' => $type, 'id_objet' => $id_objet])) {
...
function formulaires_notation_verifier_dist($objet, $id_objet) {
	...
	if (
		!sql_countsel($table_objet['table'], "$_id_objet=" . intval($id_objet))
		or (_request('content') != '')
	) {
		...
		if (
			($note < 1 || $note > notation_get_nb_notes())
