...
function notifications_post_edition($x) {
	...
	if (isset($x['args']['table'])
		...
		$notifications($quoi, $x['args']['id_objet'], []);
	}
...
function notifications_notifications_destinataires($flux) {
	...
	if (
		...
		if (in_array($id_rubrique, $limiter_rubriques)) {
			...
			while ($qui = sql_fetch($result_email)) {
				spip_log(
					$options['statut'] . ' article > admin restreint ' . $qui['id_auteur'] . ' de la rubrique' . $qui['id_rubrique'] . ' prevenu',
					'notifications'
				);
				$flux['data'][] = $qui['email'];
			}
	...
	if (
		...
		and in_array(
			$options['statut_ancien'],
			['prop', 'publie']
		) // ligne a commenter si vous voulez prevenir de la publication
	) {
	...
	if (
		...
		if (in_array($id_rubrique, $limiter_rubriques)) {
			...
			$result_email = sql_select(
				['auteurs.email', 'auteurs.id_auteur'],
				'spip_auteurs AS auteurs JOIN spip_auteurs_liens AS lien ON auteurs.id_auteur=lien.id_auteur',
	...
	if (($quoi == 'forumprive' and $GLOBALS['notifications']['thread_forum_prive'])
		...
		if ($t = $options['forum']
			...
			$s = sql_select(
				'F.email_auteur, F.notification_email, A.email',
				'spip_forum AS F LEFT JOIN spip_auteurs AS A ON F.id_auteur=A.id_auteur',
				'notification=1 AND id_thread=' . intval(
					$t['id_thread']
				) . " AND (email_auteur != '' OR notification_email != '' OR A.email IS NOT NULL) AND F.statut NOT IN ('off','spam','prop'" . $exclurepublic . ')'
			);
			...
			if (!empty($GLOBALS['notifications']['forum_article'])) {
				$s = sql_select(
					'F.email_auteur, F.notification_email, A.email',
					'spip_forum AS F LEFT JOIN spip_auteurs AS A ON F.id_auteur=A.id_auteur',
					'notification=1 AND objet=' . sql_quote($t['objet']) . ' AND id_objet=' . intval(
						$t['id_objet']
					) . " AND (email_auteur != '' OR notification_email != '' OR A.email IS NOT NULL) AND F.statut NOT IN ('off','spam','prop'" . $exclurepublic . ')'
				);
			}
	...
	if (in_array($quoi, ['forumposte', 'forumvalide', 'forumprive'])
		and $id = $flux['args']['id']
	) {
		if (isset($sent[$id])) {
...
function notifications_creer_auteur($email) {
	...
	if (!isset($a['cookie_oubli'])) {
		...
		sql_updateq('spip_auteurs', ['cookie_oubli' => $a['cookie_oubli']], 'id_auteur=' . $a['id_auteur']);
	}
