...
function action_instituer_forum_paremail_dist() {
	...
	$lien_moderation = lien_ou_expose(
		url_absolue(generer_objet_url($id_forum, 'forum', '', "forum$id_forum", false)),
		_T('notifications:info_moderation_lien_titre')
	);
	$erreur = _T('notifications:info_moderation_url_perimee') . "<br />$lien_moderation";

	if (
		$hash === _action_auteur("$action-$arg", 0, $pass, 'alea_ephemere')
		or $hash === _action_auteur("$action-$arg", 0, $pass, 'alea_ephemere_ancien')
	) {
		$erreur = '';
	}
	...
	if (!$erreur) {
		...
		if ($message) {
			if ($message['statut'] != $statut_init) {
				$erreur = _T('notifications:info_moderation_deja_faite', [
					'id_forum' => $id_forum,
	...
	$titre = (!$erreur ? _T("notifications:info_moderation_confirmee_$statut", ['id_forum' => $id_forum]) : $erreur);
...
function notifications_moderation_verifie_et_enregistre($id_forum, $statut, $email) {
	...
	if (count($moderations)) {
		foreach ($moderations as $moderation) {
			if ($moderation['statut'] == $statut) {
				...
				$erreur = _T('notifications:info_moderation_deja_faite', ['id_forum' => $id_forum, 'statut' => $moderation['statut']])
									. "<br />$lien_moderation";
			}
		}
	}
	if ($append) {
		spip_log(
			"Moderation differee a valider message $id_forum $statut par $email",
			'moderationparemail' . _LOG_INFO_IMPORTANTE
		);
		$moderations[] = ['statut' => $statut, 'time' => $_SERVER['REQUEST_TIME'], 'email' => $email];
		sql_updateq('spip_forum', ['moderation_a_valider' => json_encode($moderations)], 'id_forum=' . intval($id_forum));
	}
...
function notifications_moderation_execute($id_forum, $statut, $email, $force = false) {
	...
	if (!$erreur) {
		...
		job_queue_add(
			'notifications_moderation_valide',
			"Valider la moderation forum $id_forum statut=$statut",
			[$id_forum],
			'action/instituer_forum_paremail',
			false,
...
function notifications_moderation_valide($id_forum) {
	if ($message = sql_fetsel('*', 'spip_forum', 'id_forum=' . intval($id_forum))
	  ...
	  and $moderations = json_decode($message['moderation_a_valider'], true)) {
		...
		spip_log(
			"Valider la moderation $id_forum : " . $message['moderation_a_valider'],
			'moderationparemail' . _LOG_INFO_IMPORTANTE
		);
		...
		// sinon, il y a eu plusieurs demandes, on les refuse
			...
			spip_log(
				"Refus moderation $id_forum : " . $message['moderation_a_valider'],
				'moderationparemail' . _LOG_INFO_IMPORTANTE
			);
			$last = 0;
			foreach ($moderations as $moderation) {
			...
			// sinon on se relance dans 1min pour faire cette vidange
				...
				job_queue_add(
					'notifications_moderation_valide',
					"Purger les moderations forum $id_forum",
					[$id_forum],
					'action/instituer_forum_paremail',
					false,
		...
		if (!$moderations) {
			...
			sql_updateq('spip_forum', ['moderation_a_valider' => ''], 'id_forum=' . intval($id_forum));
		}
	}
}
