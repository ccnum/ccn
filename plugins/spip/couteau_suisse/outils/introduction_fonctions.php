...
// compatibilite avec SPIP 2.0 : la balise a fortement changé !! >> TODO
	...
	if ($id && _INTRODUCTION_LIEN) {
		if(defined('_SPIP40100')) {
			...
			$url = test_espace_prive()
					? generer_objet_url_absolue(intval($id), $racc, '', '', true, '', $connect)
					: generer_objet_url(intval($id), $racc, '', '', true, '', $connect);
		} elseif(defined('_SPIP19300')) {
			if(!$connect) $connect = true;
			$url = test_espace_prive()
				? generer_url_entite_absolue($id, $racc, '', '', $connect)
				: generer_url_entite($id, $racc, '', '', $connect);
		} else
			$url= $racc.$id;
...
function cs_introduction($texte, $descriptif, $lgr, $id, $racc, $connect) {
	...
	// si les points de suite ont été ajoutés
...
if (!function_exists('balise_INTRODUCTION')) {
	...
	function balise_INTRODUCTION($p) {
		...
		$racc = objet_type($type);
		$_id = champ_sql(id_table_objet("$racc"), $p);
		$p->code = "cs_introduction($_texte, $_descriptif, $_lgr, $_id, '$racc', \$connect)";
		#$p->interdire_scripts = true;
		$p->etoile = '*'; // propre est déjà fait dans le calcul de l'intro
		return $p;
	}
