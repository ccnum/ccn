...
function cs_liens_raccourcis_callback($matches) {
	if(isset($GLOBALS['liens_interrogation']) && $GLOBALS['liens_interrogation']) {
...
function echappe_interro_amp(&$texte) {
	return (isset($GLOBALS['liens_interrogation']) && $GLOBALS['liens_interrogation'])
		? str_replace(array('?', '!', '&amp;', '&', '--'),
...
function retour_interro_amp(&$texte) {
	$glob = isset($GLOBALS['liens_interrogation']) && $GLOBALS['liens_interrogation'];
	return (!$glob || strpos($texte, '++')===false)
		? $texte
