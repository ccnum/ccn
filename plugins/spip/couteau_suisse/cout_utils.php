...
function add_outil($tableau) {
	...
	if(!isset($tableau['id'])) {
		$tableau['id'] = 'erreur'.count($outils);
		$tableau['nom'] = couteauprive_T('erreur_id');
	}
...
function add_variable($tableau) {
	...
	if(@$tableau['format'] == _format_NOMBRE)
		$cs_variables['_nombres'][] = $nom;
	elseif(@$tableau['format'] == _format_CHAINE)
		$cs_variables['_chaines'][] = $nom;
}
...
function parse_variable_xml(&$arbre) {
	...
	if(isset($arbre['radio'])) {
		$temp = &$arbre['radio'][0];
		if(isset($temp['par_ligne']))
			$var['radio/ligne'] = $temp['par_ligne'][0];
		foreach($temp['item'] as $a)
			$var['radio'][$a['valeur'][0]] = $a['label'][0];
	}
	if(isset($arbre['label']))
		$var['label'] = $arbre['label'][0];
	if(isset($arbre['defaut_php']))
		$var['defaut'] = $arbre['defaut_php'][0];
	if(isset($arbre['code'])) foreach($arbre['code'] as $a) {
		$temp = isset($a['condition_php']) ? 'code:' . $a['condition_php'][0] : 'code';
		if(isset($a['script_php']))
...
function parse_outil_xml(&$arbre) {
	...
	if(isset($arbre['version'])) {
		...
		if(isset($temp['spip_min']))
			$out['version-min'] = $temp['spip_min'][0];
		if(isset($temp['spip_max']))
			$out['version-max'] = $temp['spip_max'][0];
	}
	return $out;
}
...
function cs_get_defaut($variable) {
	...
	if(isset($variable['externe']))
		$variable['defaut'] = $variable['externe'];
	...
	if(@$variable['format'] == _format_NOMBRE)
		$defaut = "intval($defaut)";
	elseif(@$variable['format'] == _format_CHAINE)
...
function ecrire_fichier_en_tmp(&$infos_fichiers, $type) {
	...
	if(isset($infos_fichiers['inc_' . $type]))
		foreach ($infos_fichiers['inc_' . $type] as $inc)
			$code .= "include_spip('outils/$inc');\n";
	if(isset($infos_fichiers['code_' . $type]))
		foreach ($infos_fichiers['code_' . $type] as $inline)
			$code .= $inline."\n";
	// on optimise avant...
...
function set_cs_metas_pipelines(&$infos_pipelines) {
	...
	foreach($infos_pipelines as $pipe=>$infos) {
		...
		if(is_array(@$infos['inclure'])) 
			foreach ($infos['inclure'] as $inc)
				$code .= "include_spip('$inc');\n";
		if(is_array(@$infos['inline']))
			foreach ($infos['inline'] as $inc)
				$code .= "$inc\n";
		if(is_array(@$infos['fonction']))
			foreach ($infos['fonction'] as $fonc)
...
function cs_basename($file, $suffix = '') {
	preg_match('/[^?]*/', $file, $reg);
...
function is_pipeline_outil($pipe, &$set_pipe) {
	if($ok = (strncmp('pipeline:', $pipe, 9) == 0))
		$set_pipe = trim(substr($pipe, 9));
	return $ok;
}
// est-ce que $pipe est un pipeline inline?
function is_pipeline_outil_inline($pipe, &$set_pipe) {
	if($ok = (strncmp('pipelinecode:', $pipe, 13) == 0))
		$set_pipe = trim(substr($pipe, 13));
	return $ok;
}
...
function is_traitements_outil($traitement, $fonction, &$set_traitements_utilises) {
	...
	if($ok = preg_match(',^traitement:([A-Z_]+)/?([a-z]+)?:(pre|post)_([a-zA-Z0-9_-]+)$,', $traitement, $t)) {
		...
		if($t[4]=='typo') {
			if(isset($set_traitements_utilises[$t[1]][$t[2]]['propre']))
				$t[4]='propre';
		}
...
function cs_lire_fichier_php($file) {
	...
	if($file && lire_fichier($file, $php)) {
		if(preg_match(',^<\?php(.*)?\?>$,msi', trim($php), $regs))
			return trim($regs[1]);
		return "\n" . "?>\n" . trim($php) . "\n<" . "?php\n";
	}
	return false;
...
function cs_aide_raccourci($id) {
	...
	if($outils[$id]['actif']) {
		...
		if(function_exists($f = $id . '_raccourcis'))
			return $f();
		if(!preg_match(',:aide(?:<|$),', $x = _T("couteauprive:$id:aide") ))
			return $x;
	}
	return '';
}
...
function cs_aide_raccourcis() {
	...
	foreach ($outils as $outil)
		if($a = cs_aide_raccourci($outil['id']))
			$aide[] = '<li style="margin: 0.7em 0 0 0;">&bull; ' . $a . '</li>';
	if(!count($aide))
		return '';
	// remplacement des constantes de forme @_CS_XXXX@
	$aide = preg_replace_callback(',@(_CS_[a-zA-Z0-9_]+)@,', 'cs_aide_raccourcis_callback', implode("\n", $aide));
...
function cs_aide_pipelines($outils_affiches_actifs) {
	...
	foreach ($keys as $pipe) {
		...
		$nb=0;
		foreach($outils as $outil)
			if($outil['actif'] && (isset($outil['pipeline:' . $pipe]) || isset($outil['pipelinecode:' . $pipe])))
				$nb++;
		if(($len = strlen($pipe)) > 25)
			$pipe = substr($pipe, 0, 8) . '...' . substr($pipe, $len - 14);
		if($nb)
			$aide[] = couteauprive_T('outil_nb' . ($nb > 1 ? 's' : ''), array('pipe' => $pipe, 'nb' => $nb));
	}
	// nombre d'outils actifs / interdits par les autorisations (hors versionnage SPIP)
	$nb = $ca2 = 0;
	foreach($metas_outils as $o)
		if(isset($o['actif']) && $o['actif'])
			$nb++;
	foreach($outils as $o)
		if(isset($o['interdit']) && $o['interdit'] && !cs_version_erreur($o))
			$ca2++;
	...
	return '<p><b>' . couteauprive_T('outils_actifs') . "</b> $nb"
		...
		. '</p><p style="font-size:80%; line-height: 1.4em;">' . implode("<br />", $aide) . '</p>';
}
...
function cs_nom_outil(&$outil, $traduit=true) {
	if($outil['pas_de_nom']) {
		// outil classique
		$nom = 'couteauprive:' . $outil['id'] . ':nom';
		$nom_traduit = _T($nom);
	} else {
		// outil au nom defini et traduit
		$nom = '';
		$nom_traduit = $outil['nom'];
	}
	...
	foreach ($outils as $i=>$outil) {
		...
		if($outil['actif']) {
			...
			if(defined('_SPIP30100') && $outil['categorie']=='devel' && !isset($outil['pipeline:ajouter_menus']) && !isset($outil['pipelinecode:ajouter_menus'])) {
				$infos_pipelines['ajouter_menus']['inline'][] = "\$flux = cs_creer_sousmenu(\$flux, 'menu_developpement', '$inc', '".attribut_html(cs_nom_outil($outil, false))."');";
			}
...
span.cs_BTg {font-size:140%; padding:0 0.3em;}';
	...
	if(strlen(trim($temp_css = implode("\n", $temp_css)))) {
		...
		if(strlen($temp_css) > _CS_HIT_EXTERNE) {
			...
			if(isset($cs_metas_pipelines['header_css']) && is_array($cs_metas_pipelines['header_css']))
				$temp = array_merge($temp, $cs_metas_pipelines['header_css']);
			$cs_metas_pipelines['header_css'] = $cs_metas_pipelines['header_css_prive'] = implode("\n", $temp);
		}
		unset($temp_css);
	}
	...
	$temp_js[] = "if(window.jQuery) {\nvar cs_sel_jQuery="
		...
		. "'';\nvar cs_CookiePlugin=\"<cs_html>[(#CHEMIN{javascript/jquery.cookie.js}|sinon{#CHEMIN{javascript/js.cookie.js}})]</cs_html>\";$temp_jq\n}";
...
jQuery.fn.cs_todo=function(){return this.not('.cs_done').addClass('cs_done');};\n" . implode("\n", $temp_js);
		...
		if(strlen($temp_js)>_CS_HIT_EXTERNE) {
			...
			if(isset($cs_metas_pipelines['header_js']) && is_array($cs_metas_pipelines['header_js']))
				$temp = array_merge($temp, $cs_metas_pipelines['header_js']);
			$cs_metas_pipelines['header_js'] = $cs_metas_pipelines['header_js_prive'] = implode("\n", $temp);
		}
		unset($temp_js);
	}
	...
	foreach($traitements_utilises as $bal => $balise) {
		foreach($balise as $obj => $type_objet) {
			...
			foreach($type_objet as $f => $fonction)  {
				// pas d'objet precis
				if($f===0)	$traitements_type_objet[$f] = cs_fermer_parentheses(implode("(", array_reverse($fonction)) . '(%s');
				// un objet precis
				else {
					if(!isset($liste_pivots[$f])) $liste_pivots[$f] = $f . '(%s)';
					$traitements_type_objet[$f] = !isset($fonction['pre']) ? $liste_pivots[$f]
						: str_replace('%s',
						 	 cs_fermer_parentheses(implode('(', $fonction['pre']) . '(%s'),
							 $liste_pivots[$f]
						);
					if(isset($fonction['post']))
						$traitements_type_objet[$f] = cs_fermer_parentheses(implode('(', $fonction['post']) . '(' . $traitements_type_objet[$f]);
				}
			}
			// nombre de fonctions pivot ?
			if(count($traitements_type_objet)===1) $temp = implode('', $traitements_type_objet);
			else {
		...
		$traitements_utilises[$bal] = implode("\n", $traitements_utilises[$bal]);
...
function cs_optimise_if($code, $root=true) {
	...
	foreach($regs as $r) {
		...
		$code = intval($r[1])
			? str_replace($rempl, "/* optimisation : 'IF($r[1])' */ {$temp}{$temp2}", $code)
			: str_replace($rempl, "/* optimisation : 'IF($r[1]) \{$temp\}' */{$temp2}", $code);
	}
	return $code;
}
...
// une surcharge de la fonction native est possible en ecrivant une fonction monoutil_installe()
...
function cs_recuperer_page($url,	$trans = false, $get_headers = false, $taille_max = null) {
  if(!$url) return '';
	...
	$copy = (is_string($trans) and strlen($trans) > 5); // copier le fichier ?
	$get = (!is_null($taille_max) and ($taille_max == 0)) ? 'HEAD' : 'GET';
	$options = array(
