...
function formulaires_abonner_notifications_charger_dist($email, $key, $id_auteur = null) {
	...
	[$email, $id_auteur] = notifications_email_auteur($email, $key, $id_auteur);
...
function formulaires_abonner_notifications_traiter_dist($email, $key, $id_auteur = null) {
	...
	if (_request('modifabo')) {
		...
		sql_updateq(
			'spip_forum',
			['notification' => 0],
			"notification=1 AND $where_abo AND " . sql_in('id_thread', $id_threads, 'NOT')
		);
		// abonner ceux qui sont coches
		sql_updateq(
			'spip_forum',
			['notification' => 1],
			"notification=0 AND $where_abo AND " . sql_in('id_thread', $id_threads)
		);
		$res = ['message_ok' => _T('abonnernotifications:message_abonnements_modifies')];
...
function notifications_where_abo($email, $id_auteur = null) {
	...
	if ($id_auteur) {
		$where[] = '(notification_email=' . sql_quote('') . ' AND email_auteur=' . sql_quote('') . ' AND id_auteur=' . intval(
			$id_auteur
		) . ')';
	}
