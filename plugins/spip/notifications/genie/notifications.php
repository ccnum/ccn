...
function genie_notifications_dist($time) {
	...
	$s = sql_select(
		'lien.id_auteur,lien.id_objet,lien.objet,message.titre,message.texte, message.date_heure as date, auteur.nom,auteur.email,auteur.en_ligne',
	...
	while ($t = sql_fetch($s)) {
		...
		$body .= '* ' . textebrut(propre(couper($t['titre'] . '<p>' . $t['texte'], 700))) . "\n\n";
		...
		while ($ff = sql_fetch($f)) {
			$body .= "----\n"
				. textebrut(propre(couper('** ' . $ff['titre'] . '<p>' . $ff['texte'], 700))) . "\n\n";
		}
		...
		sql_updateq(
			'spip_auteurs_liens',
			['vu' => 'oui'],
			'id_auteur=' . intval($t['id_auteur']) . ' AND objet=' . sql_quote($t['objet']) . ' AND id_objet=' . intval(
				$t['id_objet']
			)
		);
