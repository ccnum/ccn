...
class FacteurMail extends PHPMailer {
	...
	public function getMessageLog() {
		...
		if (!empty($this->to) and strpos($trace, "\nTo:") === false) {
	...
	protected function setAutoRecipients(){
		$tos = array_column($this->to, 0);
		$ccs = array_column($this->cc, 0);
		$bccs = array_column($this->bcc, 0);
		if (!empty($this->autoCc)
			&& !in_array($this->autoCc, $tos)
			&& !in_array($this->autoCc, $ccs)) {
			$this->AddCC($this->autoCc);
		}
		if (!empty($this->autoBcc)
			&& !in_array($this->autoBcc, $tos)
			&& !in_array($this->autoBcc, $ccs)
			&& !in_array($this->autoBcc, $bccs)) {
	...
	public function Send() {
		...
		$args = func_get_args();
		$res = $this->callWrapper([parent::class, 'Send'], $args);
		return $this->sendAlertIfNeeded($res);
	}

	public function addAttachment($path, $name = '', $encoding = 'base64', $type = '', $disposition = 'attachment') {
		$args = func_get_args();
		return $this->callWrapper([parent::class, 'AddAttachment'], $args);
	}

	public function AddReplyTo($address, $name = '') {
		$args = func_get_args();
		return $this->callWrapper([parent::class, 'AddReplyTo'], $args);
	}

	public function AddBCC($address, $name = '') {
		$args = func_get_args();
		return $this->callWrapper([parent::class, 'AddBCC'], $args);
	}

	public function AddCC($address, $name = '') {
		$args = func_get_args();
		return $this->callWrapper([parent::class, 'AddCC'], $args);
	}
}
