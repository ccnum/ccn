...
function formulaires_configurer_facteur_verifier_smtp(&$erreurs) {
	if (!($h = _request('smtp_host'))) {
		...
		if (!preg_match($regexp_ip_valide, $h)) { // ce n'est pas une IP
			...
			if (gethostbyaddr($h) == $h && !_request('confirm_smtp_host_ip')) {
				$erreurs['smtp_host'] = _T('facteur:erreur_ip_sans_hostname')
				  . "<br /><label><input name='confirm_smtp_host_ip' type='checkbox' value='1' /> "
				  . _T('facteur:erreur_confirm_ip_sans_hostname')
				  . "</label>";
			} else {
				// pour ne pas l'enregistrer dans la config
				set_request('confirm_smtp_host_ip');
			}
		}
