...
cs_log($metas_vars, 'metas_vars :');
	...
	foreach($variables as $var) if(autoriser('configurer', 'variable', 0, NULL, array('nom' => $var, 'outil' => $outils[$outil]))) {
		...
		$final = _request($var) ? corriger_caracteres(_request($var)) : '';
		if(in_array($var, $metas_vars['_nombres']))
			$final = intval($final);
...
cs_log(" -- donc, reinitialisation forcee !");
	// include des fichiers fonctions, si besoin, avant suppression du cache
