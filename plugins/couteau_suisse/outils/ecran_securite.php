...
function ecran_securite_pre_description_outil($flux) {
	...
	$conf = (@file_exists($f) && !defined('_SPIP20100')) || (defined("_ECRAN_SECURITE") && $flux['non'])
		? "<hr/>\n@puce@ class=\"cs_red\">"
			. _T("couteauprive:ecran_conflit" . ($flux['non']?"2":""), array("file"=>_NOM_PERMANENTS_INACCESSIBLES . "ecran_securite.php"))
			. "</span>"
		: "";
	if(defined('_ECRAN_SECURITE')) {
		...
		if($maj[0] != "-")
			$tmp = "\n" . (_ECRAN_SECURITE!=$maj
				? "- " . _T("couteauprive:ecran_maj_ko", array("n" => "<span class=\"cs_red\">$maj</span>"))
				: _T("couteauprive:ecran_maj_ok"));
	} else
		$vers = _T("couteauprive:ecran_ko");
