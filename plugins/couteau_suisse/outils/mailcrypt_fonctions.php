...
function mailcrypt_email_dist($texte) {
	if(!defined('_MAILCRYPT_TRAITE_EMAIL') || strpos($texte, '@') === false)
    return $texte;
	if(function_exists('mailcrypt_email'))
    return mailcrypt_email($texte);
...
function mailcrypt_recuperer_fond($flux) {
	...
	if(defined('_MAILCRYPT_FONDS_DEMAILCRYPT')) {
		if(!isset($fonds))
      $fonds = array_map('trim', explode(':', trim(_MAILCRYPT_FONDS_DEMAILCRYPT, ':')));
		if(in_array($flux['args']['fond'], $fonds))
      $flux['data']['texte'] = maildecrypt($flux['data']['texte']);
	}
	return $flux;
}
