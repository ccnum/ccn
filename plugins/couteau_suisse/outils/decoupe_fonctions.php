...
function onglets_callback($matches) {
	...
	return _onglets_DEBUT . implode('', $contenus) . '</div>' . _onglets_FIN;
}

// fonction appellee sur les parties du texte non comprises entre les balises : html|code|cadre|frame|script|cite|acronym
...
// fonction appellee sur les parties du textes non comprises entre les balises : html|code|cadre|frame|script|cite|acronym
...
function cs_onglets($texte){
	return cs_echappe_balises('|cite|jeux', 'cs_decoupe_compat', $texte);
}
...
function cs_decoupe($texte, $pagination_seule=false){
	// si pas de separateur, on sort
	if (strpos($texte, _decoupe_SEPARATEUR)===false) return $pagination_seule?'':$texte;
	$texte = cs_echappe_balises('|cite|table|jeux', 'decouper_en_pages_rempl', $texte, $pagination_seule);
