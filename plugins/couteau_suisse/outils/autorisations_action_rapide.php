...
function autorisations_action_rapide($actif) {
  ...
  $user = &$arr['user'];
	...
	foreach ($user as $v) {
    if(strncmp($v, 'autoriser_', 10) === 0 && preg_match(',^(autoriser_(.*?))(_dist)?$,', $v, $reg)) {
      $nb_fonctions++;
      if(!$reg[3] && function_exists($reg[1] . '_dist')) {
        $nb_surcharges++;
        continue;
      }
      $sup = ($reg[3] && function_exists($reg[1])) ? '<sup>(*)</sup>' : '';
      if(in_array($reg[1], $alias)) {
        $sp1 = '<span class="vert">';
        $sp2 = "</span>";
      } else
        $sp1 = $sp2 = '';
      ...
      elseif(strncmp($table, 'spip_', 5) === 0)
        $faire[$tmp[1] ? $tmp[1] : couteauprive_T('variable_vide')][] = $obj[$table][] =
          $tmp[1] ? $sp1 . $tmp[1] . $sup . ' ' . $tmp[0] . $sp2 : "<span class='bleu'>$sp1$tmp[0]$sp2</span>";
      elseif($tmp[1])
	...
	foreach($obj as $k => $r) {
		...
		$obj[$k] = '<li><b>' . (!$t ? "[$k]" : _T($t)) . ' : </b>' . implode(', ', $r) . '</li>';
	}
	foreach($res as $k => $r) {
		sort($r);
		$res[$k] = "<li><b>$k : </b>" . implode(', ', $r) . '</li>';
	}
	foreach($faire as $k => $r) {
		sort($r);
		$faire[$k] = "<li><b>$k : </b>" . implode(', ', $r) . '</li>';
	}
	sort($obj);
  sort($res);
  sort($faire);
