...
cs_log("decoration_installe_dist()");
	...
	return array( array(
		...
		$n ? ($n==1 ? ",<($auto_balises[0])>(.*?)</$auto_balises[0]>,ms" : ',<(' . implode('|', $auto_balises) . ')>(.*?)</\\1>,ms') : '',
...
function decoration_raccourcis() {
	...
	return _T('couteauprive:decoration:aide', array('liste' => '<b>' 
		. (is_array($racc) ? implode('</b>, <b>', $racc) : '')
		. '</b>'));
}
