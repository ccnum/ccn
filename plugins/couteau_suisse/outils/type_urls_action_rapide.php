...
//cs_log($_POST, '==== type_urls_action_rapide :'); cs_log($_GET);
	...
	foreach($fd as $i=>$f) {
		...
		$res .= ajax_action_auteur('action_rapide', $arg.$i, 'admin_couteau_suisse', "arg=type_urls|description_outil&modif=oui&cmd=descrip#cs_action_rapide", $f, '', 'function() { 
		jQuery(\'#ar_chercher\', this).click();
		}')."\n";
	}
	return $res;
}
...
function type_urls_URL_objet_exec() {
	...
	if (!$rows || !$rows[0]) return false; # Quand $id_objet n'est pas un numero connu
...
function type_urls_edit_urls_0_action() {
	// forms[0] : tout purger (cas SPIP 1.92)
...
function type_urls_edit_urls_1_action() {
	// forms[1] : editer un objet (cas SPIP 1.92)
...
function type_urls_edit_urls2_1_action() {
	...
	if(!$url) {
		...
		// pour verrouiller une url, on fixe sa date dans le futur, dans 15 ans
		$verrou = _request('ar_verrouiller')=='oui' ? 15*365.25*24*3600 : 0;
