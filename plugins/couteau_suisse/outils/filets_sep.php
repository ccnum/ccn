...
//cs_log('filets_sep_installe_dist()');
	...
	// RegExpr finale
	$filets[6] = _FILETS_REG_DEBUT . implode('|', $filets[6]) . _FILETS_REG_FIN;
...
function filets_sep_raccourcis() {
	...
	return _T('couteauprive:filets_sep:aide',
		array('liste' => '<b>' . implode('</b>, <b>', $filets[0]) . '</b>', 'max' => _FILETS_SEP_MAX_CSS));
}
...
function filets_sep_rempl($texte) {
	...
	static $filets_sep;
	if(!isset($filets_sep)) $filets_sep = cs_lire_data_outil('filets_sep');
	...
	return preg_replace_callback($filets_sep[6],
			function($matches) use($filets_sep) { return $filets_sep[1][$matches[1]]; },
			$texte);
}
