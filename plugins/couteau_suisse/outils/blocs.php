...
defined('_BLOC_TITRE_H') || define('_BLOC_TITRE_H', 'h4');
defined('_BLOC_TITRE_COUPER') || define('_BLOC_TITRE_COUPER', 30);
...
function blocs_callback($matches) {
	...
	$titre = $decoupe[0];
	$corps = isset($decoupe[1]) ? $decoupe[1] : '';
	...
	$title = function_exists('blocs_title')
		? "<div class='blocs_title blocs_invisible'>"
				. implode(_BLOC_TITLE_SEP, blocs_title($titre, $corps, $matches[2], $h<>''))
				. '</div>'
		: ''; // valeur par defaut geree en JS
	return "<div class='cs_blocs$b'><" . _BLOC_TITRE_H
		. " class='blocs_titre$h blocs_click'><a href='#_foo'>$titre</a></" . _BLOC_TITRE_H . ">"
