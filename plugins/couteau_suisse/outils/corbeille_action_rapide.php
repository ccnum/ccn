...
function corbeille_action_rapide($actif) {
	if(!$actif)
		return str_replace(':', '', _T('couteauprive:corbeille_vider'));
	foreach(cs_corbeille_table_infos() as $_table => $obj) {
		list($nb, $nb_lies, $ids, $statut, $title_lies) = cs_corbeille_gerer($_table, -1);
		...
		$lib = ($nb && defined('_SPIP30000'))
			...
			: $obj['libelle'];
		$ids = implode(',', $ids);
		$infos = $nb ? _T('couteauprive:corbeille_objets', array('nb' => $nb)) : _T('couteauprive:corbeille_objets_vide');
		if($nb_lies > 0)
			$infos .= " <span title=\"$title_lies\">" . _T('couteauprive:corbeille_objets_lies', array('nb_lies' => $nb_lies)) . '</span>';
		$objets[] = "<label><input type='checkbox' value='$_table:$ids'"
			. (($statut !== 'spam' && $nb) ? " checked='checked'" : '') 
			. " name='$_table'/>$lib.
<span class='ar_edit_info'>$infos</span></label>";
	}
	return ajax_action_rapide_simple('purge_corbeille', implode("<br/>\n", $objets), 'couteauprive:corbeille_objets_vider', 'couteauprive:corbeille_vider')
...
function cs_corbeille_table_infos($table = false) {
	...
	if(is_null($params)) {
			...
			foreach (lister_tables_objets_sql() as $sql => $o) {
				...
				if (!isset($params[$t]) && isset($o['field']['statut']) && $o['principale'] /*&& $o['editable']*/
					...
					&& ( ($t == 'auteurs' && $p = '5poubelle') || ($t == 'messages' && $p = 'poub')
						...
						|| isset($o['statut_titres'][$p = 'poubelle']) || isset($o['statut_textes_instituer'][$p = 'poubelle']) // articles
						|| isset($o['statut_titres'][$p = 'refuse']) || isset($o['statut_textes_instituer'][$p = 'refuse']) // breves, sites
		...
		if(is_array($corbeille_params))
			$params = array_merge($params, $corbeille_params);
		array_walk($params, function(&$val) { $val['libelle'] = _T($val['libelle']); });
		uasort($params, function($a, $b) { return strcmp(strtolower($a['libelle']), strtolower($b['libelle'])); });
	}
 ...
 *
 * @param nom $table
 * @param tableau $ids (si $ids == -1, on vide/compte tout)
...
//echo "<hr>$table - $type - $table_sql - $id_table";
	...
	$where = 'statut=' . sql_quote($params['statut']);
	if(is_array($ids))
		$where .= ' AND ' . sql_in($id_table, $ids);
	$ids = array_column(sql_allfetsel($id_table, $table_sql, $where), $id_table);
	if(!count($ids))
	...
	if(isset($params['tableliee']) && ($table_liee = $params['tableliee'])) {
...
//echo "<hr>$table/$unetable/",table_objet_sql($unetable),'/';print_r(array_keys($desc['field']));
			...
			if(isset($desc['field'][$id_table]))
				$nb_res = $f($desc['table'], sql_in($id_table, $ids));
			elseif(isset($desc['field']['id_objet']) && isset($desc['field']['objet']))
		 		$nb_res = $f($desc['table'], sql_in('id_objet', $ids) . " AND objet=" . sql_quote($type));
			if($nb_res) {
				$nb_lies += $nb_res;
				$title_liees[] = $desc['table'] . " ({$nb_res}x)";
			}
		}
	}
	return array($nb, $vider ? '-1' : $nb_lies, $ids, $params['statut'], implode(', ', $title_liees));
}
...
function corbeille_liste_objets_exec() {
	...
	echo '<html><head>' . f_jQuery($f(couteauprive_T('corbeille:nom')))
		. '<meta http-equiv="Content-Type" content="text/html; charset='
		. $GLOBALS['meta']['charset'] . '" /></head><body style="text-align:center">'
