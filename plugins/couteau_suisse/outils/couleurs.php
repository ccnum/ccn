...
cs_log("couleurs_installe_dist()");
	...
	if (defined('_COULEURS_SET') && _COULEURS_SET===1) {
		...
		$couleurs[0] = implode('|', array_keys($couleurs_perso));
		$aide = array_keys($couleurs_perso);
	} else {
		$aide = array_merge($couleurs[0], $couleurs[1]);
		$couleurs[0] = implode('|', $couleurs[0]);
		$couleurs[1] = implode('|', $couleurs[1]);
	}
...
function couleurs_raccourcis() {
	...
	return _T('couteauprive:couleurs:aide', array(
		'liste' => '<b>' . implode('</b>, <b>', $racc) . '</b>',
		'fond' => (defined('_COULEURS_FONDS') && _COULEURS_FONDS == 1) ? _T('couteauprive:couleurs_fonds') : '',
	));
}
...
function couleurs_BarreTypo($tr) {
	...
	if(_COULEURS_FONDS===1) {
		...
		$r2 = ' '._T('couteauprive:fonds').' '.implode(' ', $r2).'';
	} else $r2='';
...
function couleurs_PP_pre_charger($flux) {
	...
	if(defined('_COULEURS_FONDS') && _COULEURS_FONDS === 1) foreach($couleurs[2] as $i=>$v) {
...
function couleurs_nettoyer_raccourcis($texte) {
	$couleurs = cs_lire_data_outil('couleurs');
	defined('_COULEURS_SET') || define('_COULEURS_SET', 0);
