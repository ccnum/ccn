...
*/
defined('_sommaire_SANS_FOND') || define('_sommaire_SANS_FOND', '[!fond]');

function sommaire_verif_constantes() {
	// cas ou les pipelines sont appeles avant les options...
	defined('_CS_SANS_SOMMAIRE') || define('_CS_SANS_SOMMAIRE', '[!sommaire]');
	defined('_CS_AVEC_SOMMAIRE') || define('_CS_AVEC_SOMMAIRE', '[sommaire]');
}
...
function sommaire_intertitres_callback($matches) {
	...
	if(!isset($racc[$niv])) {
		// balise par defaut
		$h = cs_niveau_intertitres(true); 
		// SPIP 3.0 : plus de globale ! TODO : recuperer la valeur utilisee par TW au lieu de 'h3'
		if(!isset($GLOBALS['debut_intertitre'])) $GLOBALS['debut_intertitre'] = '<h'.$h.' class="spip">';
		if(!isset($GLOBALS['fin_intertitre'])) $GLOBALS['fin_intertitre'] = '</h'.$h.'>';
		$cfg = $niv+1;
