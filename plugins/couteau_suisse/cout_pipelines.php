...
if (defined('_LOG_CS')) {
	if (function_exists('cs_log')) {
		...
		$variable = "#CS SANS APPEL !!! Cas a verifier : inclusion cash de cout_pipelines.php";
		spip_log($variable);
...
/*
function couteau_suisse_formulaire_fond($flux) {
	return eval_metas_pipelines($flux, 'formulaire_fond');
}*/
...
function couteau_suisse_post_propre($flux) {
	...
	if (isset($GLOBALS['cs_post_propre']) && $GLOBALS['cs_post_propre']) {
...
function cs_compile_header_callback($matches) {
	if (defined('_LOG_CS'))
		cs_log(" -- compilation d'un header. Code CSS : " . cs_couper($matches[1], 150));
...
// cree un menu permettant d'acceder directement a un outil
function cs_creer_sousmenu($flux, $menu, $outil, $titre) {
	if(!autoriser('configurer', $outil)) return $flux;
	$icone = chemin_image(cs_icone(defined('_SPIP40000') ? 40 : 16));
	$url = generer_url_ecrire('admin_couteau_suisse', 'cmd=descrip&outil=@outil@#cs_infos');
	$bouton = defined('_SPIP40200')
		? new Spip\Admin\Bouton($icone, $titre, str_replace('@outil@', $outil, $url))
		: new Bouton($icone, $titre, str_replace('@outil@', $outil, $url));
	if(isset($flux[$menu]))
		$flux[$menu]->sousmenu['cs_'.$outil] = $bouton;
	elseif($menu=='menu_developpement')
		// non implemente sous SPIP, bandeau non compatible
		$flux['menu_configuration']->sousmenu['csconfig']->sousmenu['cs_'.$outil] = $bouton;
	return $flux;
}
