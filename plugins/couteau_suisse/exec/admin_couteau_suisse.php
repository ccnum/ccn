...
cs_log("INIT : enregistre_modif_outils()");
	...
	if($outil)
    $toggle[] = $outil;
  elseif(isset($_POST['cs_selection']))
    $toggle = explode(',', $_POST['cs_selection']);
  else
    return;
...
function cs_liste_docs_outils($coupe = 999, $join = "</li><li>") {
	global $metas_outils;
	$contribs = array();
	foreach($metas_outils as $o => $v)
    if(isset($v['contrib']) && isset($v['actif']) && $v['actif'])
...
cs_log("INIT : exec_admin_couteau_suisse()");
  ...
  if(!$outil = _request('outil'))
	...
	$pack = _request('pack');
	if($pack && isset($GLOBALS['cs_installer'][$pack])) {
		if($cmd == 'install'){
	...
	if($outil && $cmd == 'reset'){
		spip_log("Reset des variables de '$outil' par l'auteur id=$connect_id_auteur");
		global $outils;
	...
	$afficher_outil = ($cmd == 'descrip' || $cmd == 'switch') ? $outil : '';

	// initialisation generale forcee : recuperation de $outils;
	cs_initialisation(true, $cmd != 'noinclude');
	cs_installe_outils();
if(defined('_LOG_CS')) cs_log(" initialisation generale : OK !");

	// mise a jour des donnees si envoi via formulaire
	// sinon fait une passe de verif sur les outils
	if($cmd == 'switch' || $cmd == 'hide') {
		enregistre_modif_outils($cmd, $outil);
		cout_exec_redirige($outil ? "cmd=descrip&outil=$outil#cs_infos" : '');
	}
	...
	$cs_version_base = (isset($t['version_base']) && $t['version_base']) ? $t['version_base'] : $t['schema'];
	$cs_version = $t['version'];
	if(!function_exists('installe_un_plugin')) {
		...
		if($infos && is_array($infos) && $infos['install_test'])
			 echo $infos['install_test'][1], '<p class="cs_red">',
	...
	if(isset($GLOBALS['cs_base_update']))
		echo '<p><a href="', parametre_url(self(), 'var_mode', 'recalcul'), '"><span class="cs_red">', couteauprive_T('rafraichir'), '</span></a></p>';
	if(!$bt_version) {
		$bt_version = $get_infos($bt_dir);
		$bt_version = isset($bt_version['version']) ? $bt_version['version'] : '';
	}
	...
	if(isset($GLOBALS['cs_installer'])) foreach(array_keys($GLOBALS['cs_installer']) as $pack)
		$t .= "\n_ &bull;&nbsp;"
				. couteauprive_T('pack_du', array('pack'=>"{[{$pack}|" . couteauprive_T('pack_installe') . '->' . generer_url_ecrire($exec, 'cmd=install&pack=' . urlencode($pack)) . ']}'));
	...
	$erreur_base = (isset($GLOBALS['meta']['couteau_suisse_base_version']) && version_compare($GLOBALS['meta']['couteau_suisse_base_version'], $cs_version_base, '<'))
		? "<span class='cs_red'>DB v$cs_version_base => v" . $GLOBALS['meta']['couteau_suisse_base_version'] . ' ??</span><br/>'
		: '';
...
function quelques_verifications($bt_version) {
	// test sur jQuery
	$res = str_replace('/', '\/', addslashes(propre('<p>' . couteauprive_T('erreur:jquery') . '</p>')));
	$res = "<script type=\"text/javascript\"><!--
if(!window.jQuery) document.write('$res');
//--></script>";
	// verification d'une base venant de SPIP 1.8
	$tmp = function_exists('sql_query') ? sql_query('DESCRIBE spip_meta valeur') : spip_query('DESCRIBE spip_meta valeur');
	$tmp = function_exists('spip_fetch_array') ? spip_fetch_array($tmp) : sql_fetch($tmp);
	if(isset($tmp['Type']) && $tmp['Type'] && $tmp['Type']!='text')
		$res .= "<p class=\"cs_red\">Attention : votre base semble ancienne et le Couteau Suisse ne va pas bien fonctionner.</p><p>La table 'spip_meta' a pour type de valeur '$tmp[Type]' au lieu de 'text'.</p>";
	if(!defined('_SPIP30000')) {
		// verification de la barre typo V2 (SPIP<3.0)
		$mini = '2.5.3';
		if($bt_version && version_compare($bt_version, $mini, '<'))
...
function chargement_automatique($dir) {
	...
	if($auto && defined('_SPIP30000')) {
		...
		if($infos['id_paquet'])
			$zip = $infos['id_paquet']  .':' . $infos['id_depot'] . ':' . $dir . ':' . $zip;
	}
	...
	$res = $auto
		?	"<input type='hidden' name='$arg_chargeur' value='$zip' />"
			. "<br/><div class='cs_sobre'><input type='submit' value='&bull; "
			. attribut_html(couteauprive_T('version_update')) . "' class='cs_sobre' title='"
			. attribut_html(couteauprive_T('version_update_title')) . "' /></div>"
		: '';
	// un lien si le plugin plugin "Telechargeur" est present (SPIP < 2.0)
	if(!$res && defined('_DIR_PLUGIN_CHARGEUR'))
		$res = "<br/>&bull; <a title='" . attribut_html(couteauprive_T('version_update_chargeur_title'))
			. "' href='../spip.php?action=charger&plugin=couteau_suisse&url_retour="
			. urlencode(generer_url_ecrire('admin_couteau_suisse')) . "'>" . couteauprive_T('version_update_chargeur') . '</a>';
	return $res;
}
