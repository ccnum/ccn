...
function cs_test_fun(&$textes, $fonction) {
	...
	if (!is_callable($fonction) && !function_exists($fonction))
		return array('erreur' => $fonction. '() introuvable : outil non activ&eacute; !' . (function_exists($fonction)));
...
function cs_les_tests() {
	...
	$t['meta/couteau_suisse_version'] = $GLOBALS['meta']['couteau_suisse_version'];
...
//	$GLOBALS['meta']
	...
	$dir = dirname(!empty($_SERVER['REQUEST_URI'])
				? $_SERVER['REQUEST_URI'] 
	...
	if(function_exists('filets_sep'))
		test_outil(cs_test_fun($textes, function ($t) { return propre(filets_sep($t)); }), 'Test sur : filets_sep()');
...
//	test_outil(cs_test_fun($textes, 'typo'), 'Test sur : echappements');
	...
	// test des intertitres en image
	$textes = array(
		"\n{{{un titre 1}}}\n\n{{{*Mon titre 2}}}\n",
	);
	if(isset($metas_outils['titres_typo']['actif']) && $metas_outils['titres_typo']['actif'])
		test_outil(cs_test_fun($textes, 'propre'), 'Test sur : titres_typo_pre_typo()');
