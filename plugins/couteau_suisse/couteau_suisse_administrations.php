...
if (!isset($GLOBALS['cs_options']))
  // peut se produire lors de l'activation du plugin
  // or il nous faut les constantes de compatibilite
  include_once(dirname(__FILE__) . '/couteau_suisse_options.php');
...
function couteau_suisse_upgrade($nom_meta_base_version, $version_cible) {
  if (!isset($GLOBALS['cs_base_update']))
	...
	if (cs_le_test($current_version, $tmp, '1.12')) {
		// MAJ d'une librairie distante
		cs_maj_forcee(array(
			'masquer'
		));
		ecrire_meta($nom_meta_base_version, $current_version = $tmp);
	}
	if (cs_le_test($current_version, $tmp, '1.13')) {
		// MAJ pour rajeunissement
		cs_maj_forcee(array(
			'maj_auto'
		));
	...
	if (cs_le_test($current_version, $tmp, '1.14')) {
		// MAJ pour rajeunissement
		cs_maj_forcee(array(
			'masquer'
		));
		include_spip('inc/invalideur');
		purger_repertoire(_DIR_VAR . 'cache-js'); // pour blocs.js
		ecrire_meta($nom_meta_base_version, $current_version = $tmp);
	}
	if(!defined('_SPIP19300')) ecrire_metas(); # Pour SPIP 1.92
}
