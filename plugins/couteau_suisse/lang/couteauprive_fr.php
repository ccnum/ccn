...
@puce@ En cochant «oui» ci-dessous, le plugin surveillera l\'URL de la page afin de déplier automatiquement un bloc où se trouverait une ancre à atteindre.[[%blocs_ancre%]]
...
Cette version de SPIP utilise habituellement la balise &lt;@h3@&gt; pour les intertitres. Choisissez ici un autre remplacement :[[%racc_h1%]][[->%racc_h2%]]',
...
Cet outil peut être couplé avec « [.->sommaire] ».',
	...
	'doubleclic:description' => 'Cet outil empêche de soumettre en double les formulaires en désactivant les boutons de soumissions lors de l\'envoi. Utile pour les personnes qui "double-cliquent".',
	'doubleclic:nom' => 'Pas de double-clic',
...
Attention, cet outil a besoin pour fonctionner du plugin {jQuery} : «{Round Corners}». Si le fichier distant est correctement installé et si vous cochez la case suivante, le Couteau Suisse peut l\'activer automatiquement. [[%jcorner_plugin%]]',
	...
	'label:blocs_ancre' => 'Déplier un bloc contenant une ancre :',
	...
	'label:quota_cache_nb' => 'Nombre de fichiers cache utilisés :',
	...
	'left_join:description' => '[Expérimental] Met à votre disposition le critère "<code>{left_join}</code>" permettant à vos boucles d\'utiliser des jointures SQL "<code>LEFT JOIN</code>" à la place des jointures d\'intersection "<code>INNER JOIN</code>" que SPIP utilise par défaut.
...
<quote><cadre>// sans paramètre, le critère s\'applique à toutes les tables liées
<BOUCLE(AUTEURS spip_documents){left_join}...

// les paramètres (séparés par des espaces) permettent de cibler certaines tables
<BOUCLE(spip_documents_liens documents rubriques){left_join documents}...

// avec un paramètre "debug", des informations utiles sont affichées
// (à éviter en mode production)
...
Note : les paquets <code>.zip</code> n\'étant pas reconstruits instantanément, il se peut que vous soyez obligé d\'attendre un certain délai avant de pouvoir effectuer la totale mise à jour d\'un plugin tout récemment modifié.',
	...
	'maj_librairies_ko' => 'Librairies introuvables, tentez de refaire votre action ?',
...
-* Articles : @_ART@.',
	...
	// O
	'obf_liens:description' => "Met à votre disposition le filtre : '|obfusquer_liens'. L'obfuscation de liens permet d'empêcher les robots des moteurs de recherche de suivre les liens de type <code><a href=\"page.html\">par ici</a></code>, sans modifier la navigation. Cela pourra éviter la transmission de pagerank non désiré vers des pages à moindre intérêt, et guider les robots vers des pages plus pertinentes et riches.\n\nEn plaçant <code>#FILTRE{obfusquer_liens}</code> dans un squelette, tous les liens seront obfusqués.\n\nA noter : si le Javascript est désactivé sur la page, le lien sera inactif et l'internaute ne pourra donc pas le suivre.",
	'obf_liens:nom' => 'Obfusquer des liens',
...
<q1>• Utilisez le caractère «*» pour plusieurs chiffres, «?» pour un seul et les crochets pour des classes de chiffres.</q1>',
	...
	'spip_cache:description4' => '@puce@ Le cache occupe un certain espace disque et SPIP peut en limiter l\'importance (SPIP 3.2 maxi). Une valeur vide ou égale à 0 signifie qu\'aucun quota ne s\'applique.[[%quota_cache% Mo]]',
	'spip_cache:description5' => '@puce@ Le cache occupe un certain espace disque et SPIP peut contrôler le nombre de fichiers utilisés (SPIP 4.0 mini). Par défaut, SPIP utilise 16^4 (65 536) fichiers répartis en 16^2 (256) répertoires. Un chiffre entre 2 et 8 vous permet de modifier le nombre de fichiers cache entre : 16^2 (256) et 16^8 (4 294 967 296). [[16^%quota_cache_nb% fichiers (actuellement : @_CS_CACHE_NB@ fichiers et @_CS_CACHE_NBR@ répertoires)]]',
	'spip_cache:description' => '@puce@ Lorsqu\'une modification du contenu du site est faite, SPIP invalide immédiatement le cache sans attendre le calcul périodique suivant. Si votre site a des problèmes de performance face à une charge très élevée, vous pouvez cocher « non » à cette option.[[%derniere_modif_invalide%]]
...
@puce@ Configurez les options de journalisation grâce à l\'outil «[.->spip_log]».',
	...
	'switchcase:nom' => 'Balises #SWITCH #CASE',
	'switchcase:description' => 'Les balises <code>#SWITCH</code>, <code>#CASE</code> et <code>#CASE_DEFAULT</code>, ainsi que le filtre <code>|switchcase</code>, implémentent les contrôles <code>switch/case/default</code> du PHP ou du C.

Ces écritures simplifient grandement la lisibilté de vos squelettes. Exemples :
<quote><cadre>#SWITCH{#ENV{code_erreur}}
[(#CASE{0}) Tout va bien]
[(#CASE{1}) Attention ?]
[(#CASE{2}) Oups !]
...
@puce@ Après un appel à <code>#SWICH{valeur comparée}</code>, <code>#CASE{valeur calculée}</code> vaut <code>\'\'</code> (vide) si la valeur calculée est différente de la valeur comparée, et vaut  <code>\' \'</code> (espace) en cas d\'égalité.

@puce@ La valeur testée par le <code>#SWICH</code> reste disponible grâce à : <code>#GET{_switch_}</code>.

@puce@ On ne peut pas emboiter plusieurs <code>#SWITCH</code> dans un même squelette.',
...
@puce@ {{Ordre des groupes dans le formulaire d\'ajout de mots-clés}} [[%tri_groupes%]][[->%tri_perso_groupes%]]',
	...
	'trousse_balises:description' => 'Outil (en cours de développement) qui vous offre quelques balises très simples et bien pratiques pour améliorer la lisibilité de vos squelettes.
