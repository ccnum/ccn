...
function description_outil2($outil_id) {
	...
	if (!$outil['id'] || !autoriser('configurer', 'outil', 0, NULL, $outil))
	...
	if ($temp = cs_action_fichiers_distants($outil) . cs_action_rapide($outil_id, $actif))
	...
	if ($actif && isset($outil['code:spip_options']) && $outil['code:spip_options'] && ($outil_id <> 'cs_comportement'))
	...
	if (isset($outil['auteur']) && $outil['auteur'])
	...
	return (defined('_AUTOBR') && _AUTOBR) ? str_replace(_AUTOBR, ' ', $p) : $p;
...
function cs_span_color($texte, $color = 'green') {
  return '<span style="color:' . $color . '">' . $texte . '</span>';
}
...
function cs_action_fichiers_distants(&$outil, $forcer = false, $tester = false) {
	if (!isset($outil['fichiers_distants']))
		return '';
  static $zz = 1;
	$lib    = sous_repertoire(_DIR_RACINE, 'lib', false, true);
	$actif  = $outil['actif'];
	$retour = array();
	$reload = false;
	foreach ($outil['fichiers_distants'] as $i) {
    ...
    $fichier_distant = &$outil[$i];
		$dir      = $lib ? sous_repertoire($lib, $outil['id'], false, true) : '';
    if (!$dir) {
      $lib = '';
      $forcer = true;
      $dir    = sous_repertoire(sous_repertoire(_DIR_CS_TMP, 'lib'), $outil['id']);
      $erreur = $statut = cs_span_color(couteauprive_T('lib_inaccessible', array('outil' => $outil['id'])), 'red');
    }
    // prefixe et basename sans arguments
    preg_match('/[^:]*/', $i, $reg);
    $f    = $reg[0] . '_' . cs_basename($fichier_distant);
    ...
    elseif ($actif || $forcer) {
      if ($distant = cs_recuperer_page($fichier_distant)) {
        // 2e appel : envoi du texte
        $distant = pipeline('fichier_distant', array(
          'outil' => $outil['id'],
          'fichier_local' => $file,
          'fichier_distant' => $fichier_distant,
          'message' => '',
          'texte' => $distant,
          'actif' => $actif
        ));
        ...
        if (preg_match(',\.php\d?$,', $file)) {
          ...
          if (!@eval("return true; " . preg_replace(',function\s+\w+,', '\\0_zz' . $zz++, $test)))
            $distant = false;
          else
      ...
      if ($distant) {
        if ($lib)
          $statut = cs_span_color(couteauprive_T('distant_charge'), 'green');
        $reload = true;
      } else {
        if ($lib)
          $erreur = $statut = cs_span_color(couteauprive_T('distant_echoue'), 'red');
      }
    } else
      // outil inactif et pas de forcage
      $erreur = $statut = couteauprive_T('distant_inactif');

		if ($erreur) {
			$outil['erreurs']['fichiers_distants'][$fichier_distant] = -1;
			$message .= "\n_ " . couteauprive_T('local_ko', array(
				'file' => cs_root_canonicalize($file)
			));
		} else
      unset($outil['erreurs']['fichiers_distants'][$fichier_distant]);
		$retour[] = '[{' . basename($file) . "}->$fichier_distant]\n_ " . $statut . $message;
	}
	if ($tester)
		return $retour;
	...
	$retour = '<ul style="margin:0.6em 0 0.6em 4em;"><li>' . implode("</li><li style='margin-top:0.4em;'>", $retour) . '</li></ul>';
	$actualiser = ($actif || !$erreur) ? 'rss_actualiser' : ($erreur ? 'distant_charger' : '');

	$actualiser = $actualiser ? "\n<p class='cs_sobre'><input class='cs_sobre' type='submit' value=\" [" . attribut_html(couteauprive_T('' . $actualiser)) . ']" />' . $reload . '</p>' : '';
	...
	return ajax_action_auteur('action_rapide', 'fichiers_distants', 'admin_couteau_suisse', "arg=$outil[id]|fichiers_distants&cmd=descrip#cs_action_rapide",
			  ...
			  . '<div>' . propre($retour) . '</div>' . $actualiser);
}
...
function cs_raccourcis_presents($champs, $racc) {
	...
	foreach ($champs as $c) {
		...
		$ids = sql_allfetsel($objet, table_objet_sql($type), $champ . ' LIKE ' . _q("$racc"));
		if(count($ids)) {
			$fct   = function($a) use(&$type) { return '[' . $a . '->' . $type . $a . ']'; };
			$ids   = array_map($fct, array_column($ids, $id_objet));
			$res[] = implode(', ', $ids);
		}
	}
