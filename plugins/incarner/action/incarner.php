			...
			if ($login = array_shift($arg)){
				...
				if ($auteur = auth_identifier_login($login, '')
					...
					if (autoriser('incarner', 'auteur', $auteur['id_auteur'])){
						...
						$id_auteur_session = (isset($GLOBALS['visiteur_session']['id_auteur']) ? $GLOBALS['visiteur_session']['id_auteur'] : 0);
						spip_log("DEBUT Incarnation compte auteur $login #".$auteur['id_auteur']." par #$id_auteur_session", 'incarner' . _LOG_INFO_IMPORTANTE);
		...
		case 'logout':
			if ($id_auteur = incarner_racine_incarnation()) {
				...
				if ($auteur = auth_identifier_login($login, '')
					...
					$id_auteur_session = (isset($GLOBALS['visiteur_session']['id_auteur']) ? $GLOBALS['visiteur_session']['id_auteur'] : 0);
					spip_log("FIN Incarnation compte auteur #$id_auteur_session par $login #$id_auteur", 'incarner' . _LOG_INFO_IMPORTANTE);
