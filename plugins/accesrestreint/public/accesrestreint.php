	...
	if (
		...
		switch ($boucle->type_requete) {
			case 'hierarchie':
				...
				$boucle->select = array_merge($boucle->select, [$t, $primary]); // pour postgres
			...
			case 'petitions':
				...
				$boucle->select = array_merge($boucle->select, [$t, $primary]);
				// Test sur l'objet lui-même
				$boucle->where[] = accesrestreint_objets_accessibles_where($boucle->id_table, $primary);
				// Test sur la hiérarchie
			...
			case 'signatures':
				...
				$t = $t . '.id_article';
				$boucle->select = array_merge($boucle->select, [$t]);
				// Test sur l'objet lui-même
				$boucle->where[] = accesrestreint_objets_accessibles_where($boucle->id_table, $primary);
				// Test sur la hiérarchie
				$boucle->where[] = accesrestreint_articles_accessibles_where($t);
...
function accesrestreint_forums_accessibles_where($primary, $_publique = '') {
	...
	if (test_plugin_actif('breves')) {
		...
		// si la table des brèves n'existe pas ou est vide ou il n'y a pas de forum attachés à des brèves
		// on peut ignorer toute vérification la concernant
		...
		if (
			$trouver_table('spip_breves') !== null
			and sql_countsel('spip_breves')
			and sql_countsel('spip_forum', 'objet="breve"')
		) {
			accesrestreint_signaler_plugin_absent(_T('accesrestreint:alerte_table_breves_forum'));
		}
	}
...
function accesrestreint_documents_accessibles_where($primary, $_publique = '') {
	...
	if (test_plugin_actif('breves')) {
		...
		// sinon il faut avertir qu'il serait peut être bon d'activer le plugin brève sous peine de "démasquer" des documents liés à des brèves protégées
		$trouver_table = charger_fonction('trouver_table', 'base');
		if (
			$trouver_table('spip_breves') !== null
			and sql_countsel('spip_breves')
			and sql_countsel('spip_documents_liens', 'objet="breve"')
		) {
			accesrestreint_signaler_plugin_absent(_T('accesrestreint:alerte_table_breves_documents'));
		}
	}
...
function accesrestreint_signaler_plugin_absent($message) {
	static $deja = [];
	if (!$deja[$message]) {
		$deja[$message] = true;
